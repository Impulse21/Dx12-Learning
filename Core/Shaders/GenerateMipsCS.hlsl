        ...
        case WIDTH_HEIGHT_EVEN:
            {
                ...
                Src1 = SrcMip.SampleLevel(LinearClampSampler, UV, GenerateMipsCB.SrcMipLevel);
            }
        break;
        case WIDTH_ODD_HEIGHT_EVEN:
            {
                ...
                // horizontally.
                float2 UV1 = GenerateMipsCB.TexelSize * (IN.DispatchThreadID.xy + float2(0.25, 0.5));
                float2 Off = GenerateMipsCB.TexelSize * float2(0.5, 0.0);
 
                Src1 = 0.5 * (SrcMip.SampleLevel(LinearClampSampler, UV1, GenerateMipsCB.SrcMipLevel) +
                   SrcMip.SampleLevel(LinearClampSampler, UV1 + Off, GenerateMipsCB.SrcMipLevel));
            }
            break;
        case WIDTH_EVEN_HEIGHT_ODD:
            {
                ...
                // Use 2 bilinear samples to guarantee we don't undersample when downsizing by more than 2x
                // vertically.
                float2 UV1 = GenerateMipsCB.TexelSize * (IN.DispatchThreadID.xy + float2(0.5, 0.25));
                float2 Off = GenerateMipsCB.TexelSize * float2(0.0, 0.5);
        ...
        case WIDTH_HEIGHT_ODD:
            {
                ...
                // in both directions.
                float2 UV1 = GenerateMipsCB.TexelSize * (IN.DispatchThreadID.xy + float2(0.25, 0.25));
                float2 Off = GenerateMipsCB.TexelSize * 0.5;
 
                Src1 = SrcMip.SampleLevel(LinearClampSampler, UV1, GenerateMipsCB.SrcMipLevel);
                Src1 += SrcMip.SampleLevel(LinearClampSampler, UV1 + float2(Off.x, 0.0), GenerateMipsCB.SrcMipLevel);
                Src1 += SrcMip.SampleLevel(LinearClampSampler, UV1 + float2(0.0, Off.y), GenerateMipsCB.SrcMipLevel);
                Src1 += SrcMip.SampleLevel(LinearClampSampler, UV1 + float2(Off.x, Off.y), GenerateMipsCB.SrcMipLevel);
                Src1 *= 0.25;
            }
            break;
    }
